...
public static class State
{
    public static ComputeState GetCompState(PopState state, Simulator simulator, SimParams simParams)
    {
        ...
        if (simulator.TotalCloneCount >= simParams.MaxClones && simParams.MaxClones > 0)
        {
            return ComputeState.Finished;
        }
        if (state.Alive <= 0)
    ...
    public static string StateLog(int repeatId, int tryNo, Simulator simulator, SimParams simParams, List<PopState> popStates)
    {
        ...
        return $"sim: {repeatId + 1}.{tryNo}/{simParams.Reps}, " +
               ...
               $"SC_total: {simulator.TotalCloneCount}, " +
    ...
    public static (List<Clone> subClones, ListTree tree) GetMullerData(IEnumerable<Clone> clones, SimParams simParams, List<PopState> popStates)
    {
        if (simParams.CalcFish)
        {
            var cloneList = clones.ToList();
